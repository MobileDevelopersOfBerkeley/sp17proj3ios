...
class FeedViewController: UIViewController {
    ...
    func setupButton() {
        newPostButton = UIButton(frame: CGRect(x: 10, y: 90, width: UIScreen.main.bounds.width - 20, height: 50))
        newPostButton.setTitle("New Social", for: .normal)
        newPostButton.setTitleColor(UIColor.gray, for: .normal)
...
extension FeedViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row == 0{
            cell.eventImage.image = UIImage(named: "food")
        }
        else {
