...
class NewSocialViewController: UIViewController {
    ...
    func createNewPost(sender: UIButton!) {
        ...
        storage.put(eventImageData!, metadata: metadata).observe(.success) { (snapshot) in
            ...
            let newPost = ["eventName": nameOfEvent, "poster": self.currentUser?.name as Any, "userImageUrl": self.currentUser?.imageUrl as Any, "interestedPeople": self.interestedUsers, "posterId": self.currentUser?.id! as Any, "description": eventDescription, "date": eventDate, "eventImageUrl": url!, "eventId": key2] as [String : Any]
